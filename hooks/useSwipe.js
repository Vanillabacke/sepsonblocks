// https://github.com/antonioru/beautiful-react-hooks/blob/master/src/useSwipe.ts

import { MutableRefObject, useRef, useState } from 'react'
import useMouseEvents from './useMouseEvents'
import useTouchEvents from './useTouchEvents'
import { Direction, getDirection, getHorizontalDirection, getPointerCoordinates, getVerticalDirection } from '../utils/swipeUtils'

const defaultOptions = {
  direction: 'both',
  threshold: 10,
  preventDefault: false,
  preventDefaultDirection: [],
}
...
const useSwipe = (targetRef = null, options = defaultOptions) => {
  ...
  const continueSwipe = (event) => {
    ...
    if( options.preventDefaultDirection.includes( state.direction) ) {
        event.preventDefault()
        event.stopPropagation()
    }
