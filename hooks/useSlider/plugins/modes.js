...
export default function Free(slider) {
    ...
    function snap() {
        ...
        if (details.slides[track.absToRel(idx)].portion === 0)
            idx -= direction;
        if (startIdx !== moveIdx) {
            idx = moveIdx;
        }
        if (sign(track.idxToDist(idx, true)) !== direction)
            idx += direction;
        idx = clampIdx(idx);
