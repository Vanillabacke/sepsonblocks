import { dir, elem, elems, Events, getProp, rect, setAttr, } from '../core/utils';
export default function Web(container, defaultOptions) {
    return (slider) => {
        ...
        function applyAttributes(remove) {
            setAttr(slider.container, 'reverse', dir(slider.container) === 'rtl' && !remove ? '' : null);
            setAttr(slider.container, 'v', slider.options.vertical && !remove ? '' : null);
            setAttr(slider.container, 'disabled', slider.options.disabled && !remove ? '' : null);
        }
        ...
        function checkBreakpoint() {
            ...
            slider.options = Object.assign(Object.assign({}, options), _options);
            applyAttributes();
            updateSize();
            updateSlides();
            renewTrackConfig();
        ...
        function getElementSize(elem) {
            ...
            return ((slider.options.vertical ? sizes.height : sizes.width) / slider.size ||
                1);
        }
        ...
        function initTrack(idx) {
            ...
            slider.track.init(idx !== null && idx !== void 0 ? idx : (details ? details.abs : 0));
        }
        ...
        function renewTrackConfig() {
            ...
            const slidesCount = typeof slides === 'number'
                ? slides
                : getProp(slides, 'number', elemsCount, true);
            const config = [];
            const perView = getProp(slides, 'perView', 1, true);
            const spacing = getProp(slides, 'spacing', 0, true) / slider.size || 0;
            const spacingPortion = perView === 'auto' ? spacing : spacing / perView;
            const originOption = getProp(slides, 'origin', 'auto');
            let length = 0;
            for (let i = 0; i < slidesCount; i++) {
                const size = perView === 'auto'
                    ? getElementSize(elems[i])
                    : 1 / perView - spacing + spacingPortion;
                const origin = originOption === 'center'
                    ? 0.5 - size / 2
                    : originOption === 'auto'
                        ? 0
                        : originOption;
        ...
        function resize() {
            ...
            if (slider.options.disabled || newSize === compareSize)
                return;
            compareSize = newSize;
            update();
        }
