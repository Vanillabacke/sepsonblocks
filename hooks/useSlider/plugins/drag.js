import { clamp, elems, Events, prevent, setAttr, sign, stop, } from '../core/utils';
export default function Drag(slider) {
    const events = Events();
    ...
    function preventScrolling(element) {
        let start;
        events.input(element, 'touchstart', (e) => {
            start = xy(e);
            scrollLock = true;
        ...
        events.input(element, 'touchmove', (e) => {
            ...
            const scrollingEnabled = (vertical && element.style.overflowY === 'scroll') ||
    ...
    function setSpeed() {
        ...
        dragSpeed =
            typeof speed === 'function' ? speed : val => val * speed;
    }
    function isVertical() {
        return slider.options.vertical;
    }
    function preventClicks() {
        const attr = `data-slider-clickable`;
    ...
    function update() {
        events.purge();
        if (!slider.options.drag || slider.options.disabled)
            return;
        setSpeed();
        ...
        const attr = 'data-slider-scrollable';
        elems(`[${attr}]:not([${attr}=false])`, slider.container).map(element => preventScrolling(element));
    }
