...
export default function Track(slider) {
    ...
    function setRange() {
        ...
        if (dragMax === null)
            loopMax = maxIdx;
        if (getProp(rangeOption, 'align', false) &&
            maxIdx !== infinity &&
    ...
    function details() {
        ...
        const slideDetails = slides.map((slide, idx) => {
            ...
            const viewPortPortion = Math.max(distanceViewport >= 0 && end <= 1
                ? 1
                : end < 0 || distanceViewport > 1
                    ? 0
                    : distanceViewport < 0
                        ? Math.min(1, (slide[0] + distanceViewport) / slide[0])
                        : (1 - distanceViewport) / slide[0], 0);
    ...
    function getIndexes(pos) {
        ...
        const origin = relativePositions.indexOf([...relativePositions].reduce((a, b) => Math.abs(b - positionRelative) < Math.abs(a - positionRelative) ? b : a));
        let idx = origin;
        if (positionSign < 0 && positionRelative !== 0)
            factor++;
    ...
    function velocity() {
        ...
        const data = measurePoints.reduce((acc, next) => {
            const { distance } = next;
            const { timestamp } = next;
            if (timestampNow - timestamp > 200)
                return acc;
            ...
            if (acc.lastTimestamp)
                acc.time += timestamp - acc.lastTimestamp;
    ...
    function idxToDist(idx, absolute, fromPosition) {
        let distance;
        if (absolute || !getLoop())
            return absoluteIdxToDist(idx, fromPosition);
        if (!idxInRange(idx))
            return null;
        const { abs, rel } = getIndexes(fromPosition !== null && fromPosition !== void 0 ? fromPosition : position);
        const idxDistance = idx - rel;
        const nextIdx = abs + idxDistance;
        distance = absoluteIdxToDist(nextIdx);
        const otherDistance = absoluteIdxToDist(nextIdx - slidesCount * Math.sign(idxDistance));
    ...
    function initSlides() {
        ...
        relativePositions = slides.reduce((acc, val) => {
            ...
            distance -= val[2];
            if (acc[acc.length - 1] > distance)
                distance = acc[acc.length - 1];
            acc.push(distance);
            return acc;
        }, null);
